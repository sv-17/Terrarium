...
// Obtaining the POSTED values for company names and logo
...
<html lang="en">
  <head>
    ...
    <link rel="stylesheet" href="s-company-tests.css">
  </head>
  <body>
      ...
      <div class="-quiz-instructions-container">
        ...
        >
          ...
          >
            ...
            >
              ...
              >
              <?php echo '<span class="-quiz-instructions-text">Hi ' . $_SESSION['username'] . '</span>'; ?>
              <img
                  alt="avatar"
                  src="<?=$pfp?>"
        ...
        <div class="companydetails">
            <div class="logocompany">
                <img class="imglogo" src=<?=$companyLogo?>>
            </div>
            <div class="">
                <p class="datacompany"> TESTS FOR <?=strtoupper($companyName)?> </p>
            </div>
        </div>
        <div class="modalc">
        <button class="tablink" onclick="openPage('Home', this, '#ebebeb')" id="defaultOpen">Un-Attempted</button>
        <button class="tablink" onclick="openPage('News', this, '#ebebeb')">Attempted</button>
        
        <div id="Home" class="tabcontent">
        <?php
  // Prepare and execute the SQL query
  $query = "SELECT t.Test_ID, t.Test_Name FROM tests t
            LEFT JOIN user_tests ut ON t.Test_ID = ut.Test_ID
            WHERE ut.Test_Status = 0 AND ut.username = ? AND t.Test_Company = ?";

  $stmt = $conn->prepare($query);
  $stmt->bind_param("ss", $_SESSION['username'], $companyName);
  $stmt->execute();

  // Retrieve the results
  $result = $stmt->get_result();

  // Iterate over the results and process each row
  while ($row = $result->fetch_assoc()) {
    ...
    echo '<b><a href="s-quiz-instructions.php?testname=' . urlencode($testName) . '&testid=' . $testID . '&companyname=' . urlencode($_SESSION['comp_name']) . '" target="_blank">' . $testName . '</a></b>';
    echo '</p>';
...
?>
  ...
  $query = "SELECT t.Test_ID, t.Test_Name FROM tests t
            LEFT JOIN user_tests ut ON t.Test_ID = ut.Test_ID
            WHERE ut.Test_Status = 1 AND ut.username = ? AND t.Test_Company = ?";
  $stmt = $conn->prepare($query);
  $stmt->bind_param("ss", $_SESSION['username'], $companyName);
  $stmt->execute();
